...
void Cpu::initOpcodeTable() {
    ...
    opcodeTable[0xCB] = {"CB Prefix", &Cpu::handler_CB_PREFIX, 0};
    ...
    opcodeTable[0xE0] = {"LD (a8), A", &Cpu::handler_LD_a8, 12};
    ...
    opcodeTable[0xEA] = {"LD (a16), A", &Cpu::handler_LD_a16, 16};
    ...
    opcodeTable[0xF0] = {"LD A, (a8)", &Cpu::handler_LD_a8, 12};
    ...
    opcodeTable[0xF8] = {"LD HL, SP+s8", &Cpu::handler_LD_HL_SP_s8, 12};
    opcodeTable[0xF9] = {"LD SP, HL", &Cpu::handler_LD_SP_HL, 8};
    opcodeTable[0xFA] = {"LD A, (a16)", &Cpu::handler_LD_a16, 16};
 ...
 */
void Cpu::writeReg8(uint8_t opcode, uint8_t data) {
    bool useHL = ((opcode & 0b111)) == 0x6;

    if (useHL)
        writeByteMemory(data, HL.val);
    else
        *(decode_reg8(opcode)) = data;
}

void Cpu::writeReg8(uint8_t opcode, char type, uint8_t data) {
    bool useHL =
        ((type == 'd') ? ((opcode >> 3) & 0b111) : (opcode & 0b111)) == 0x6;

    if (useHL)
        writeByteMemory(HL.val, data);
    else
        *(decode_reg8(opcode, type)) = data;
}
/*
...
void Cpu::handler_LD_a16(uint8_t opcode) {
    uint16_t addr = fetch16();
    if (opcode == 0xEA)
        writeByteMemory(addr, AF.hi);
    else
        AF.hi = readByteMemory(addr);
}

void Cpu::handler_LD_a8(uint8_t opcode) {
...
void Cpu::handler_LD_HL_SP_s8(uint8_t opcode) {
    uint8_t u8 = fetch();
    int8_t s8 = static_cast<int8_t>(u8);
    uint16_t sum = SP.val + s8;

    writeFlag(Flag::Z, 0);
    writeFlag(Flag::N, 0);
    writeFlag(Flag::H, ((SP.val & 0xF) + (s8 & 0xF)) > 0xF);
    writeFlag(Flag::C, ((SP.val & 0xFF) + s8) > 0xFF);

    HL.val = sum;
}
void Cpu::handler_LD_SP_HL(uint8_t opcode) { SP.val = HL.val; }
...
/*
 * Layouut of a CB prefixed opcode:
 *
 * prefixed by 0xCB
 * following 8 bits:
 *
 * xxxx xyyy
 *
 * 5 x-bits determine instruction
 * 3 y-bits determine register
 *
 */
void Cpu::handler_CB_PREFIX(uint8_t opcode) {
    uint8_t newOpcode = fetch();
    uint8_t regVal = readReg8(newOpcode, 's');
    uint8_t opcodeGroup = (newOpcode >> 6) & 0b11;
    uint8_t opcodeName = (newOpcode >> 3) & 0b111;

    switch (opcodeGroup) {
    case 0x0:
        switch (opcodeName) {
        case 0x0:
            Cpu::CB_RLC_r8(regVal);
            break;
        case 0x1:
            Cpu::CB_RRC_r8(regVal);
            break;
        case 0x2:
            Cpu::CB_RL_r8(regVal);
            break;
        case 0x3:
            ...
            break;
        case 0x4:
            Cpu::CB_SLA_r8(regVal);
            break;
        case 0x5:
            Cpu::CB_SRA_r8(regVal);
            break;
        case 0x6:
            Cpu::CB_SWAP_r8(regVal);
            break;
        case 0x7:
            Cpu::CB_SRL_r8(regVal);
            break;
        }
        writeReg8(newOpcode, 's', regVal);
        break;
    case 0x1: {
        uint8_t bit = (newOpcode >> 3) & 0b111;
        Cpu::BIT_n_r8(regVal, bit);
    } break;
    case 0x2: {
        uint8_t bit = (newOpcode >> 3) & 0b111;
        Cpu::RES_n_r8(regVal, bit);
        writeReg8(newOpcode, 's', regVal);
    } break;
    case 0x3: {
        uint8_t bit = (newOpcode >> 3) & 0b111;
        Cpu::SET_n_r8(regVal, bit);
        writeReg8(newOpcode, 's', regVal);
    } break;
    }
}

void Cpu::CB_RLC_r8(uint8_t &val) {
    rotateLeftCarry(val);
    writeFlag(Flag::Z, val == 0);
    writeFlag(Flag::H, 0);
    writeFlag(Flag::N, 0);
    t_cycles += 8;
}
void Cpu::CB_RRC_r8(uint8_t &val) {
...
void Cpu::CB_SLA_r8(uint8_t &val) { shiftLeftArithmetic(val); }
void Cpu::CB_SRA_r8(uint8_t &val) { shiftRightArithmetic(val); }
void Cpu::CB_SWAP_r8(uint8_t &val) { swapNibbles(val); }
void Cpu::CB_SRL_r8(uint8_t &val) { shiftRightLogical(val); }
void Cpu::BIT_n_r8(uint8_t &val, uint8_t bit) { copyBit(val, bit); }
void Cpu::RES_n_r8(uint8_t &val, uint8_t bit) { resetBit(val, bit); }
void Cpu::SET_n_r8(uint8_t &val, uint8_t bit) { setBit(val, bit); }
